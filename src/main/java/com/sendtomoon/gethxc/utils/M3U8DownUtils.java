...
public class M3U8DownUtils {
	...
	public M3U8DownUtils(String originUrlpath, String preUrlPath, String rootPath) {
	...
	class DownloadThread extends Thread {
		...
		public void run() {
			for (int i = startIndex; i <= endIndex; i++) {
				...
				String fileOutPath = folderPath + File.separator + i + ".ts";
				keyFileMap.put(i, fileOutPath);
				String message = "%s: 线程 " + (startIndex / (endIndex - startIndex) + 1) + ", " + (i + 1) + "/"
						+ urlList.size() + ", 合计: %d";
				try {
					downloadNet(preUrlPath + subUrlPath, fileOutPath);
